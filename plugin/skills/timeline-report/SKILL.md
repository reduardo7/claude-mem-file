...
description: Generate a "Journey Into [Project]" narrative report analyzing a project's entire development history from claude-mem-file's timeline. Use when asked for a timeline report, project history analysis, development journey, or full project report.
---

# Timeline Report

Generate a comprehensive narrative analysis of a project's entire development history using claude-mem-file's persistent memory timeline.
...
The claude-mem-file worker must be running on localhost:37777. The project must have claude-mem-file observations recorded.
...
Use Bash to fetch the complete timeline from the claude-mem-file worker API:
...
Deploy an Agent (using the Task tool) with the full timeline and the following analysis prompt. Pass the ENTIRE timeline as context to the agent. The agent should also be instructed to query the SQLite database at `~/.claude-mem-file/claude-mem-file.db` for the Token Economics section.

**Agent prompt:**

```
You are a technical historian analyzing a software project's complete development timeline from claude-mem-file's persistent memory system. The timeline below contains every observation, session boundary, and summary recorded across the project's entire history.

You also have access to the claude-mem-file SQLite database at ~/.claude-mem-file/claude-mem-file.db. Use it to run queries for the Token Economics & Memory ROI section. The database has an "observations" table with columns: id, memory_session_id, project, text, type, title, subtitle, facts, narrative, concepts, files_read, files_modified, prompt_number, discovery_tokens, created_at, created_at_epoch, source_tool, source_input_summary.
...
7. **Memory and Continuity** -- How did persistent memory (claude-mem-file itself, if applicable) affect the development process? Were there moments where recalled context from prior sessions saved significant time or prevented repeated mistakes?

8. **Token Economics & Memory ROI** -- Quantitative analysis of how memory recall saved work:
   - Query the database directly for these metrics using `sqlite3 ~/.claude-mem-file/claude-mem-file.db`
...
- **Worker not running:** "The claude-mem-file worker is not responding on port 37777. Start it with your usual method or check `ps aux | grep worker-service`."
