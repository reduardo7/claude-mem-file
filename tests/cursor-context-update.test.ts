 ...
 * .cursor/rules/claude-mem-file-context.mdc for registered projects.
 *
...
describe('Cursor Context Update', () => {
  ...
  describe('writeContextFile', () => {
    ...
    it('creates claude-mem-file-context.mdc file', () => {
      writeContextFile(workspacePath, 'test context');
...
- Added new feature`;
    ...
    it('includes footer with MCP tools mention', () => {
      ...
      const content = readContextFile(workspacePath);
      expect(content).toContain("Use claude-mem-file's MCP search tools for more detailed queries");
    });

    it('uses atomic write (no temp file left behind)', () => {
      writeContextFile(workspacePath, 'test context');

      const tempFile = join(workspacePath, '.cursor', 'rules', 'claude-mem-file-context.mdc.tmp');
