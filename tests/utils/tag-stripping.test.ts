 ...
 *
 * Tests the tag privacy system for <private>, <claude-mem-file-context>, and <system_instruction> tags.
...
describe('Tag Stripping Utilities', () => {
  ...
  describe('stripMemoryTagsFromPrompt', () => {
    describe('basic tag removal', () => {
      ...
      it('should strip single <claude-mem-file-context> tag', () => {
        const input = 'public content <claude-mem-file-context>injected context</claude-mem-file-context> more public';
        const result = stripMemoryTagsFromPrompt(input);
        expect(result).toBe('public content  more public');
      });

      it('should strip both tag types in mixed content', () => {
        const input = '<private>secret</private> public <claude-mem-file-context>context</claude-mem-file-context> end';
        const result = stripMemoryTagsFromPrompt(input);
    ...
    describe('multiple tags handling', () => {
      ...
      it('should strip multiple <claude-mem-file-context> blocks', () => {
        const input = '<claude-mem-file-context>ctx1</claude-mem-file-context><claude-mem-file-context>ctx2</claude-mem-file-context> content';
        const result = stripMemoryTagsFromPrompt(input);
      ...
      it('should handle many interleaved tags', () => {
        let input = 'start';
        for (let i = 0; i < 10; i++) {
          input += ` <private>p${i}</private> <claude-mem-file-context>c${i}</claude-mem-file-context>`;
        }
        input += ' end';
        const result = stripMemoryTagsFromPrompt(input);
        // Tags are stripped but spaces between them remain
        expect(result).not.toContain('<private>');
        expect(result).not.toContain('<claude-mem-file-context>');
    ...
    describe('empty and private-only prompts', () => {
      ...
      it('should return empty string for entirely context-tagged prompt', () => {
        const input = '<claude-mem-file-context>all is context</claude-mem-file-context>';
        const result = stripMemoryTagsFromPrompt(input);
        expect(result).toBe('');
      });
      ...
      it('should handle whitespace-only after stripping', () => {
        const input = '<private>content</private>   <claude-mem-file-context>more</claude-mem-file-context>';
        const result = stripMemoryTagsFromPrompt(input);
        expect(result).toBe('');
      });
    });
...
secret
      ...
      it('should strip multiline content within <claude-mem-file-context> tags', () => {
        const input = `start
...
finish`;
  ...
  describe('stripMemoryTagsFromJson', () => {
    describe('JSON content stripping', () => {
      ...
      it('should strip claude-mem-file-context tags from JSON', () => {
        const jsonContent = JSON.stringify({
          data: '<claude-mem-file-context>injected</claude-mem-file-context> real data'
        });
        const result = stripMemoryTagsFromJson(jsonContent);
        const parsed = JSON.parse(result);
      ...
      it('should handle tool_response with tags', () => {
        const toolResponse = {
          output: 'result <claude-mem-file-context>context data</claude-mem-file-context>',
          status: 'success'
        };
...
after`;
  ...
  describe('system-reminder tag stripping', () => {
    ...
    it('should strip <system-reminder> mixed with other tag types', () => {
      const input = '<system-reminder>reminder</system-reminder> public <private>secret</private> <claude-mem-file-context>ctx</claude-mem-file-context> end';
      const result = stripMemoryTagsFromPrompt(input);
      expect(result).toBe('public   end');
    });
