...
describe('Cursor MCP Configuration', () => {
  ...
  describe('configureCursorMcp', () => {
    ...
    it('adds claude-mem-file server with correct structure', () => {
      configureCursorMcp(mcpJsonPath, mcpServerPath);

      const config: CursorMcpConfig = JSON.parse(readFileSync(mcpJsonPath, 'utf-8'));

      expect(config.mcpServers).toBeDefined();
      expect(config.mcpServers['claude-mem-file']).toBeDefined();
      expect(config.mcpServers['claude-mem-file'].command).toBe('node');
      expect(config.mcpServers['claude-mem-file'].args).toEqual([mcpServerPath]);
    });

    it('preserves existing MCP servers when adding claude-mem-file', () => {
    ...
    it('updates existing claude-mem-file server path', () => {
      // First config
      ...
      expect(config.mcpServers['claude-mem-file'].args).toEqual([newPath]);
    });
  ...
  describe('removeMcpConfig', () => {
    it('removes claude-mem-file server from config', () => {
      configureCursorMcp(mcpJsonPath, mcpServerPath);
      removeMcpConfig(mcpJsonPath);

      const config: CursorMcpConfig = JSON.parse(readFileSync(mcpJsonPath, 'utf-8'));
      expect(config.mcpServers['claude-mem-file']).toBeUndefined();
    });

    it('preserves other servers when removing claude-mem-file', () => {
      // Setup: both servers
      mkdirSync(join(tempDir, '.cursor'), { recursive: true });
      const config = {
        mcpServers: {
          'other-server': { command: 'python', args: ['/path.py'] },
          'claude-mem-file': { command: 'node', args: ['/mcp.cjs'] }
        }
      };
      ...
      expect(updated.mcpServers['claude-mem-file']).toBeUndefined();
    });
    ...
    it('does nothing if claude-mem-file not in config', () => {
      mkdirSync(join(tempDir, '.cursor'), { recursive: true });
      const config = {
        mcpServers: {
          'other-server': { command: 'python', args: ['/path.py'] }
  ...
  describe('path handling', () => {
    it('handles absolute path with spaces', () => {
      ...
      const config: CursorMcpConfig = JSON.parse(readFileSync(mcpJsonPath, 'utf-8'));
      expect(config.mcpServers['claude-mem-file'].args).toEqual([pathWithSpaces]);
    });
    ...
    it('handles Windows-style path', () => {
      ...
      const config: CursorMcpConfig = JSON.parse(readFileSync(mcpJsonPath, 'utf-8'));
      expect(config.mcpServers['claude-mem-file'].args).toEqual([windowsPath]);
    });
    ...
    it('handles path with special characters', () => {
      ...
      const config: CursorMcpConfig = JSON.parse(readFileSync(mcpJsonPath, 'utf-8'));
      expect(config.mcpServers['claude-mem-file'].args).toEqual([specialPath]);

      // Verify it survives JSON round-trip
      const reread: CursorMcpConfig = JSON.parse(readFileSync(mcpJsonPath, 'utf-8'));
      expect(reread.mcpServers['claude-mem-file'].args![0]).toBe(specialPath);
    });
  });
});
