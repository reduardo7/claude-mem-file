...
Connects claude-mem-file to Cursor so that:
...
claude-mem-file settings set CLAUDE_MEM_PROVIDER gemini
claude-mem-file settings set CLAUDE_MEM_GEMINI_API_KEY your-api-key

# Option B: OpenRouter (free models available)
claude-mem-file settings set CLAUDE_MEM_PROVIDER openrouter
claude-mem-file settings set CLAUDE_MEM_OPENROUTER_API_KEY your-api-key
```
...
- claude-mem-file provides search tools via MCP
...
- Check logs: `tail -f ~/.claude-mem-file/logs/worker-$(date +%Y-%m-%d).log`
...
- Visit [claude-mem-file docs](https://docs.claude-mem-file.ai) for full feature set
