...
Use claude-mem-file's persistent memory in Cursor without a Claude Code subscription. Choose between free-tier providers (Gemini, OpenRouter) or paid options.
...
Since you don't have Claude Code, you need to configure an AI provider for claude-mem-file's summarization engine.
...
```bash
# From the claude-mem-file repo directory (recommended - all projects)
...
- Visit https://docs.claude-mem-file.ai for full documentation
...
$settingsDir = "$env:USERPROFILE\.claude-mem-file"
