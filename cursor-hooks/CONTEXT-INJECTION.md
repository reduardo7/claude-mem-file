...
1. **Install**: `claude-mem-file cursor install` creates initial context file
2. **Stop hook**: `session-summary.sh` updates context after each session ends
3. **Cursor**: Automatically includes `.cursor/rules/claude-mem-file-context.mdc` in all chats
...
3. Fetches existing context from claude-mem-file and writes to `.cursor/rules/claude-mem-file-context.mdc`
...
cat > .cursor/rules/claude-mem-file-context.mdc << EOF
---
alwaysApply: true
---
# Memory Context
...
Located at: `.cursor/rules/claude-mem-file-context.mdc`
...
[Your context from claude-mem-file appears here]
...
When you run `claude-mem-file cursor install`, the project is registered in `~/.claude-mem-file/cursor-projects.json`. This allows the worker to automatically update your context file whenever a new summary is generated - even if it happens from Claude Code or another IDE working on the same project.

To see registered projects:
```bash
cat ~/.claude-mem-file/cursor-projects.json
```
...
| Initial setup | Automatic | `claude-mem-file cursor install` creates initial context |
| MCP tool access | ✅ Full support | ✅ Full support |
| Web viewer | ✅ Available | ✅ Available |

## First Session Behavior

When you run `claude-mem-file cursor install`:
...
Configure claude-mem-file's MCP server in Cursor for search tools:
...
Ask the agent: "Check claude-mem-file for any previous work on authentication"

## File Location

The context file is created at:
```
<workspace>/.cursor/rules/claude-mem-file-context.mdc
```

This is version-controlled by default. Add to `.gitignore` if you don't want to commit it:
```
.cursor/rules/claude-mem-file-context.mdc
```
