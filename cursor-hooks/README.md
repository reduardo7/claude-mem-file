# Claude-Mem-File Cursor Hooks Integration
...
The hooks bridge Cursor's hook system to claude-mem-file's worker API, allowing:
...
1. **Install**: `claude-mem-file cursor install` generates initial context
2. **Stop hook**: Updates context in `.cursor/rules/claude-mem-file-context.mdc` after each session
...
- **MCP Tools**: Configure claude-mem-file's MCP server for `search`, `timeline`, `get_observations` tools
...
claude-mem-file cursor install user
...
| --------------------- | --------------------- | -------------------------------------- |
| `beforeSubmitPrompt`  | `session-init.sh`     | Initialize claude-mem-file session     |
...
- Initializes a new session in claude-mem-file using `conversation_id` as the session ID
- Extracts project name from workspace root
- Outputs `{"continue": true}` to allow prompt submission

### Context Hook (`context-inject.sh`)

- Ensures claude-mem-file worker is running before session
- Outputs `{"continue": true}` to allow prompt submission
...
- Maps them to claude-mem-file's observation format
...
- Requests summary generation from claude-mem-file
- **Updates context file** in `.cursor/rules/claude-mem-file-context.mdc` for next session

## Configuration

The hooks read configuration from `~/.claude-mem-file/settings.json`:
...
2. Check worker logs:
   ...
   ```bash
   tail -f ~/.claude-mem-file/logs/worker-$(date +%Y-%m-%d).log
   ```

3. Restart worker:
   ```bash
   claude-mem-file restart
   ```
...
- [Claude-Mem-File Documentation](https://docs.claude-mem-file.ai)
- [Cursor Hooks Reference](../docs/context/cursor-hooks-reference.md)
- [Claude-Mem-File Architecture](https://docs.claude-mem-file.ai/architecture/overview)
