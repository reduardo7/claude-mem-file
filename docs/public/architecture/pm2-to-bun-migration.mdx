...
This document describes the PM2 to Bun migration that occurred in v7.1.0 (December 2025). If you're installing claude-mem-file for the first time, this migration has already been completed and you can use the current Bun-based system documented in the main guides.
...
Claude-mem-file version 7.1.0 introduces two major architectural migrations:
...
- **Process Name**: `claude-mem-file-worker`
...
- **Logs**: `~/.pm2/logs/claude-mem-file-worker-*.log`
- **PID File**: `~/.pm2/pids/claude-mem-file-worker.pid`

**Lifecycle Commands**:
```bash
pm2 start <script>           # Start worker
pm2 stop claude-mem-file-worker   # Stop worker
pm2 restart claude-mem-file-worker # Restart worker
pm2 delete claude-mem-file-worker  # Remove from PM2
pm2 logs claude-mem-file-worker    # View logs
```
...
- **Logs**: `~/.claude-mem-file/logs/worker-YYYY-MM-DD.log`
- **PID File**: `~/.claude-mem-file/.worker.pid`
- **Port File**: `~/.claude-mem-file/.worker.port` (new)
...
- Database file: `~/.claude-mem-file/claude-mem-file.db` (same location)
...
if (!existsSync(pm2MigratedMarker)) {
  try {
    spawnSync('pm2', ['delete', 'claude-mem-file-worker'], { stdio: 'ignore' });
...
`ensureWorkerRunning()` checks if `~/.claude-mem-file/.worker.pid` exists (it doesn't for first run after update)
</Step>
<Step title="Start Worker Decision">
Worker not running → Call `startWorker()`
</Step>
<Step title="Migration Check">
Check if `~/.claude-mem-file/.pm2-migrated` exists
</Step>
<Step title="PM2 Cleanup">
Execute `pm2 delete claude-mem-file-worker` (errors ignored), create marker file
</Step>
...
**Location**: `~/.claude-mem-file/.pm2-migrated`
...
4. **PM2 cleanup**: `pm2 delete claude-mem-file-worker` (old worker terminated)
5. **Marker creation**: `~/.claude-mem-file/.pm2-migrated` with timestamp
...
| `pm2 stop claude-mem-file-worker` | `npm run worker:stop` | Stop worker |
| `pm2 restart claude-mem-file-worker` | `npm run worker:restart` | Restart worker |
| `pm2 delete claude-mem-file-worker` | `npm run worker:stop` | Remove worker |
| `pm2 logs claude-mem-file-worker` | `npm run worker:logs` | View logs |
| `pm2 describe claude-mem-file-worker` | `npm run worker:status` | Detailed status |
...
**Logs**:
```
Old: ~/.pm2/logs/claude-mem-file-worker-out.log
     ~/.pm2/logs/claude-mem-file-worker-error.log

New: ~/.claude-mem-file/logs/worker-YYYY-MM-DD.log
```

**PID Files**:
```
Old: ~/.pm2/pids/claude-mem-file-worker.pid

New: ~/.claude-mem-file/.worker.pid
```
...
New: ~/.claude-mem-file/.worker.pid
     ~/.claude-mem-file/.worker.port
     ~/.claude-mem-file/.pm2-migrated (all platforms)
```

**Database** (unchanged):
```
Same: ~/.claude-mem-file/claude-mem-file.db
```
...
├────┼────────────────────┼─────────┼─────────┼──────────┤
│ 0  │ claude-mem-file-worker  │ online  │ 0       │ 2d 5h    │
└────┴────────────────────┴─────────┴─────────┴──────────┘
```
...
# Or just remove claude-mem-file logs
rm -f ~/.pm2/logs/claude-mem-file-worker-*.log
rm -f ~/.pm2/pids/claude-mem-file-worker.pid
```
...
```
~/.claude-mem-file/
├── claude-mem-file.db          # Database (unchanged)
...
~/.pm2/
├── logs/
│   ├── claude-mem-file-worker-out.log
│   └── claude-mem-file-worker-error.log
├── pids/
│   └── claude-mem-file-worker.pid
└── pm2.log
```
...
```
~/.claude-mem-file/
├── claude-mem-file.db          # Database (same file)
├── chroma/                # Vector embeddings (unchanged)
├── logs/
...
**Symptoms**:
- `pm2 list` still shows `claude-mem-file-worker`
...
```bash
# Manual cleanup
pm2 delete claude-mem-file-worker
pm2 save  # Persist the deletion

# Force re-migration (optional)
rm ~/.claude-mem-file/.pm2-migrated
...
**Manual Resolution**:
```bash
rm ~/.claude-mem-file/.worker.pid
rm ~/.claude-mem-file/.worker.port
...
# 2. Start PM2 worker
pm2 start plugin/scripts/worker-cli.js --name claude-mem-file-worker
...
# 5. Verify migration
pm2 list  # Should NOT show claude-mem-file-worker
cat ~/.claude-mem-file/.pm2-migrated  # Should exist
