---
title: "Configuration"
description: "Environment variables and settings for Claude-Mem-File"
---
...
| `CLAUDE_MEM_OPENROUTER_APP_NAME`             | `claude-mem-file`                | Optional: App name for analytics      |
...
| Setting                       | Default                         | Description                           |
|-------------------------------|---------------------------------|---------------------------------------|
| `CLAUDE_MEM_DATA_DIR`         | `~/.claude-mem-file`                 | Data directory location               |
...
```bash
./claude-mem-file-settings.sh
```

This script manages settings in `~/.claude-mem-file/settings.json`.

### Manual Configuration

Edit `~/.claude-mem-file/settings.json`:
...
- **Production (installed plugin)**: `~/.claude-mem-file/` (always, regardless of CLAUDE_PLUGIN_ROOT)
- **Development**: Can be overridden with `CLAUDE_MEM_DATA_DIR`

```
~/.claude-mem-file/
├── claude-mem-file.db           # SQLite database
...
```json
{
  "description": "Claude-mem-file memory system hooks",
  "hooks": {
    "SessionStart": [{
      "hooks": [{
        "type": "command",
...
Claude-Mem-File provides MCP search tools for querying your project history.
...
Claude-Mem-File supports switching between stable and beta versions via the web viewer UI.
...
Claude-mem-file can automatically generate `CLAUDE.md` files in your project folders with activity timelines. This feature is disabled by default.
...
Claude-Mem-File injects past observations into each new session, giving Claude awareness of recent work. You can configure exactly what gets injected using the **Context Settings Modal**.
...
Settings can be customized in `~/.claude-mem-file/settings.json`.
...
Control which tools are excluded from observations. Edit `~/.claude-mem-file/settings.json`:
```json
{
