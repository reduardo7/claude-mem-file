...
description: "How claude-mem-file intercepts file reads to save tokens using observation history"
---
...
Projects matching patterns in `CLAUDE_MEM_EXCLUDED_PROJECTS` skip the gate entirely. Configure this in `~/.claude-mem-file/settings.json`:
...
2. Find the claude-mem-file hooks section under `hooks.PreToolUse` and remove the entry with the `Read` matcher.
...
The File Read Gate is one piece of claude-mem-file's layered context strategy:
