...
description: "Add persistent memory to Gemini CLI with claude-mem-file"
---
...
Gemini CLI starts every session from scratch. Claude-mem-file changes that by capturing observations, decisions, and patterns — then injecting relevant context into each new session.

<Info>
**How it works:** Claude-mem-file installs lifecycle hooks into Gemini CLI that capture tool usage, agent responses, and session events. A local worker service extracts semantic observations and injects relevant history at session start.
</Info>
...
### Step 1: Install claude-mem-file
...
Claude-mem-file needs an AI provider to extract observations from your sessions. Choose one:
...
npx claude-mem-file status

# Check hooks are installed — look for claude-mem-file entries
cat ~/.gemini/settings.json | grep claude-mem-file
```
...
Launch Gemini CLI normally. Claude-mem-file works in the background:

```bash
gemini
```

On session start, you'll see claude-mem-file context injected with your recent observations and project history.

## What gets captured

Claude-mem-file registers 8 of Gemini CLI's 11 lifecycle hooks:
...
1. Verify hooks exist in settings:
   ...
   ```
   You should see entries like `"SessionStart"`, `"AfterTool"`, etc. with claude-mem-file commands.
...
# View logs
npx claude-mem-file logs

# Restart worker
npx claude-mem-file restart
```
...
3. Check your AI provider is configured in `~/.claude-mem-file/settings.json`

### Raw escape codes in output

If you see characters like `[31m` or `[0m` in the session context, your claude-mem-file version may need updating:
