...
The OpenClaw plugin gives claude-mem-file persistent memory to agents running on the [OpenClaw](https://openclaw.ai) gateway. It handles three things:

1. **Observation recording** — Captures tool usage from OpenClaw's embedded runner and sends it to the claude-mem-file worker for AI processing
...
<Info>
OpenClaw's embedded runner (`pi-embedded`) calls the Anthropic API directly without spawning a `claude` process, so claude-mem-file's standard hooks never fire. This plugin bridges that gap by using OpenClaw's event system to capture the same data.
</Info>
...
The observation feed sends a formatted message to your OpenClaw channel every time claude-mem-file creates a new observation. Each message includes the observation title and subtitle so you can follow along as your agents work.
...
Add the `observationFeed` block to your claude-mem-file plugin config in your OpenClaw gateway configuration:
...
Add `claude-mem-file` to your OpenClaw gateway's plugin configuration:
...
<Note>
The claude-mem-file worker service must be running on the same machine as the OpenClaw gateway. The plugin communicates with it via HTTP on `localhost:37777`.
</Note>
...
<ParamField body="workerPort" type="number" default={37777}>
  Port for the claude-mem-file worker service. Override if your worker runs on a non-default port.
</ParamField>
...
The plugin uses HTTP calls to the already-running claude-mem-file worker service rather than spawning subprocesses. This means:
...
Each OpenClaw agent session gets a unique `contentSessionId` (format: `openclaw-<sessionKey>-<timestamp>`) that maps to a claude-mem-file session in the worker. The plugin tracks:
...
- Claude-mem-file worker service running on `localhost:37777` (or configured port)
