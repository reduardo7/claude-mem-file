...
description: "Common issues and solutions for Claude-Mem-File"
---
...
1. Check browser localStorage is enabled:
   ```javascript
   // In browser console
   localStorage.getItem('claude-mem-file-settings')
   ```
...
3. Clear localStorage and try again:
   ```javascript
   // In browser console
   localStorage.removeItem('claude-mem-file-settings')
   ```
...
1. Clear install cache:
   ```bash
   rm ~/.claude/plugins/marketplaces/reduardo7/.install-version
   ```
...
3. Check version marker:
   ```bash
   cat ~/.claude/plugins/marketplaces/reduardo7/.install-version
   cat ~/.claude/plugins/marketplaces/reduardo7/package.json | grep version
   ```
...
4. Verify new port:
   ```bash
   cat ~/.claude-mem-file/worker.port
   ```
...
3. Check database for corruption:
   ```bash
   sqlite3 ~/.claude-mem-file/claude-mem-file.db "PRAGMA integrity_check;"
   ```
...
3. Verify database has observations:
   ```bash
   sqlite3 ~/.claude-mem-file/claude-mem-file.db "SELECT COUNT(*) FROM observations;"
   ```
...
1. Check if summaries exist:
   ```bash
   sqlite3 ~/.claude-mem-file/claude-mem-file.db "SELECT COUNT(*) FROM session_summaries;"
   ```
...
3. Check database size (large database = slow queries):
   ```bash
   ls -lh ~/.claude-mem-file/claude-mem-file.db
   ```

4. Optimize database:
   ```bash
   sqlite3 ~/.claude-mem-file/claude-mem-file.db "VACUUM;"
   ```
...
3. Check package.json exists:
   ```bash
   ls -la ~/.claude/plugins/marketplaces/reduardo7/package.json
   ```
...
2. Check for stale locks:
   ```bash
   lsof ~/.claude-mem-file/claude-mem-file.db
   ```
...
2. Backup database:
   ```bash
   cp ~/.claude-mem-file/claude-mem-file.db ~/.claude-mem-file/claude-mem-file.db.backup
   ```
...
1. Check FTS5 tables exist:
   ```bash
   sqlite3 ~/.claude-mem-file/claude-mem-file.db "SELECT name FROM sqlite_master WHERE type='table' AND name LIKE '%_fts';"
   ```
...
3. Check triggers exist:
   ```bash
   sqlite3 ~/.claude-mem-file/claude-mem-file.db "SELECT name FROM sqlite_master WHERE type='trigger';"
   ```
...
2. Verify FTS5 tables populated:
   ```bash
   sqlite3 ~/.claude-mem-file/claude-mem-file.db "SELECT COUNT(*) FROM observations_fts;"
   ```
...
**Symptoms**: `/plugin install claude-mem-file` fails.
...
3. Verify installation:
   ```bash
   ls -la ~/.claude/plugins/marketplaces/reduardo7/
   ```
...
**Solution**: Check `~/.claude-mem-file/` exists and is writable.
...
2. **Create issue**: [GitHub Issues](https://github.com/reduardo7/claude-mem-file/issues)
...
- [Configuration](configuration) - Customize Claude-Mem-File
