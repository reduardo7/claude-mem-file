...
description: "Configure Claude-Mem-File behavior and language with the Mode System"
---

# Modes & Languages

Claude-Mem-File uses a flexible **Mode System** to adapt its behavior, observation types, and output language. This allows you to switch between different workflows (like coding vs. email investigation) or languages without reinstalling the plugin.
...
Set the active mode using the `CLAUDE_MEM_MODE` setting in `~/.claude-mem-file/settings.json`:
