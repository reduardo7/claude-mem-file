...
Claude-mem-file supports Google's Gemini API as an alternative to the Claude Agent SDK for extracting observations from your sessions. This can significantly reduce costs since Gemini offers a generous free tier.
...
If Gemini is selected but encounters errors, claude-mem-file automatically falls back to the Claude Agent SDK:
...
Either:
- Set `CLAUDE_MEM_GEMINI_API_KEY` in `~/.claude-mem-file/settings.json`, or
...
Claude-mem-file enforces these limits automatically with built-in delays between requests. Processing may be slower but stays within limits.
...
<Tip>
**Recommended**: Enable billing on your Google Cloud project to unlock much higher rate limits. You won't be charged unless you exceed the generous free quota. This allows claude-mem-file to process observations instantly instead of waiting between requests.
</Tip>
