...
Claude-mem-file provides persistent memory across sessions through **4 MCP tools** that follow a token-efficient **3-layer workflow pattern**.

## Overview

Instead of fetching all historical data upfront (expensive), claude-mem-file uses a progressive disclosure approach:
...
**MCP Server:** Located at `~/.claude/plugins/marketplaces/reduardo7/plugin/scripts/mcp-server.cjs`

**Worker Service:** Express API on port 37777, managed by Bun

**Database:** SQLite FTS5 full-text search on `~/.claude-mem-file/claude-mem-file.db`
