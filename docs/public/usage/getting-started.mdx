...
description: "Learn how Claude-Mem-File works automatically in the background"
---

# Getting Started with Claude-Mem-File

## Automatic Operation

Claude-Mem-File works automatically once installed. No manual intervention required!
...
Every time Claude uses a tool, claude-mem-file captures it:
...
Context is stored in SQLite database at `~/.claude-mem-file/claude-mem-file.db`.
...
Claude-Mem-File supports sessions that span multiple user prompts:
...
Claude-Mem-File provides MCP tools for querying your project history. Simply ask naturally:
