...
Knowledge agents let you compile a slice of your claude-mem-file observation history into a **queryable "brain"** that answers questions conversationally. Instead of getting raw search results back, you get synthesized, grounded answers drawn from your actual project history -- decisions, discoveries, bugfixes, and features.
...
```
build_corpus name="hooks-expertise" query="hooks architecture" project="claude-mem-file" limit=200
```
...
Under the hood, `CorpusBuilder` searches your observations, hydrates full records, parses structured fields (facts, concepts, files), calculates stats, and writes everything to `~/.claude-mem-file/corpora/hooks-expertise.corpus.json`.
