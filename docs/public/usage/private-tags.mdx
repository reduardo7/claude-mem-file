...
Use `<private>` tags to mark content you don't want persisted in claude-mem-file's observation database. This gives you fine-grained control over what gets remembered across sessions.
...
- Claude's responses (not captured by claude-mem-file)
...
4. **Test it works**: Check `~/.claude-mem-file/silent.log` if you're unsure whether tags are being stripped
...
2. Check the database to ensure private content is not stored:
   ```bash
   # Check user prompts
   sqlite3 ~/.claude-mem-file/claude-mem-file.db "SELECT prompt_text FROM user_prompts ORDER BY created_at_epoch DESC LIMIT 1;"

   # Check observations
   sqlite3 ~/.claude-mem-file/claude-mem-file.db "SELECT narrative FROM observations ORDER BY created_at_epoch DESC LIMIT 1;"
   ```
...
2. Check `~/.claude-mem-file/silent.log` for errors
...
If you see errors in `~/.claude-mem-file/silent.log`:
```
[save-hook] stripMemoryTags received non-string: { type: 'object' }
```

This is usually harmless - it indicates defensive type checking is working. However, if you see these frequently, it may indicate a bug. Please report it at https://github.com/thedotmack/claude-mem-file/issues
