...
description: Use claude-mem-file memory search in Claude Desktop with MCP tools
icon: desktop
---

<Note>
**Availability:** Claude-mem-file MCP tools work with Claude Desktop on macOS and Windows.
</Note>

## Overview

Claude Desktop can access your claude-mem-file memory database through **MCP tools**. This allows you to search past sessions, decisions, and observations directly from Claude Desktop conversations.

## Prerequisites

Before configuring MCP tools, ensure:

1. **claude-mem-file is installed** and the worker service is running
    ...
    ```json
    {
      "mcpServers": {
        "mcp-search": {
          "command": "node",
          "args": [
            "/Users/YOUR_USERNAME/.claude/plugins/marketplaces/reduardo7/plugin/scripts/mcp-server.cjs"
          ]
        }
      }
    }
    ...
    ```json
    {
      "mcpServers": {
        "mcp-search": {
          "command": "node",
          "args": [
            "C:\\Users\\YOUR_USERNAME\\.claude\\plugins\\marketplaces\\reduardo7\\plugin\\scripts\\mcp-server.cjs"
          ]
        }
      }
    }
...
1. Ensure claude-mem-file has recorded sessions (check http://localhost:37777)
2. Verify the database exists: `ls ~/.claude-mem-file/claude-mem-file.db`
