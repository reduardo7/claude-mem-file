# How Claude-Mem-File Uses Hooks: A Lifecycle-Driven Architecture
...
Claude-Mem-File is fundamentally a **hook-driven system**. Every piece of functionality happens in response to lifecycle events:
...
**Key insight:** Claude-Mem-File doesn't interrupt or modify Claude Code's behavior. It observes from the outside and provides value through lifecycle hooks.
...
Claude-Mem-File had several architectural constraints:
...
Claude-Mem-File uses lifecycle hook scripts across 5 lifecycle events. SessionStart runs 3 hooks in sequence: smart-install, worker-service start, and context-hook.
...
<AccordionGroup>
  ...
  <Accordion title="Observations not captured">
    ...
    **Debugging:**
    1. Check database: `sqlite3 ~/.claude-mem-file/claude-mem-file.db "SELECT * FROM observation_queue"`
...
**Privacy guarantees:**
- All data stored locally in `~/.claude-mem-file/claude-mem-file.db`
...
*The hook-driven architecture enables Claude-Mem-File to be both powerful and invisible. Users never notice the memory system working - it just makes Claude smarter over time.*
