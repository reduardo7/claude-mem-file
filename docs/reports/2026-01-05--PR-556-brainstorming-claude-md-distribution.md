Brainstorming Report: CLAUDE.md Distribution Architecture
  ...
  Potential Claude Code feature request - support command substitution in context config:
  {
    "context": {
      "sources": [
        { "command": "claude-mem-file live-context ${CWD}" }
      ]
    }
  }
  Or folder-level .claude-context files containing just:
  exec: claude-mem-file live-context .
  ...
  - Timeline data in separate file: claude-mem-file-timeline.csv
  - Generated fresh on SessionStart
  - Block Claude from reading timeline file via .claude/settings.local.json: "ignorePaths": ["claude-mem-file-timeline.csv"]
  ...
  - Writes claude-mem-file-timeline.csv and includes in push
  ...
  - Post-pull hook: claude-mem-file sync --from-timeline
  ...
  - claude-mem-file-timeline.csv = Machine-generated context sync (blocked from reading, already injected)
