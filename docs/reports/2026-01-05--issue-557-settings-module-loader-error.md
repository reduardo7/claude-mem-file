...
**Date:** January 5, 2026
**Issue:** [#557](https://github.com/reduardo7/claude-mem-file/issues/557) - Plugin fails to start: settings.json not generated, worker throws module loader error
...
2. **Manually test context-hook with Bun:**
   ```bash
   bun ~/.claude/plugins/marketplaces/reduardo7/plugin/scripts/context-hook.js
   ```

3. **Manually test context-hook with Node (should fail):**
   ```bash
   node ~/.claude/plugins/marketplaces/reduardo7/plugin/scripts/context-hook.js
   ```
...
5. **Verify worker can start:**
   ```bash
   bun ~/.claude/plugins/marketplaces/reduardo7/plugin/scripts/worker-service.cjs start
   ```
