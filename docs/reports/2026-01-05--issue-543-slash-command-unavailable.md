# Issue #543 Analysis: /claude-mem-file Slash Command Not Available Despite Installation
...
### Finding 1: No `/claude-mem-file` Slash Command Exists
...
2. **Skills, not commands**: Claude-mem-file uses Claude Code's **skill system**, not the command system. Skills are defined in `plugin/skills/`:
   ...
   - `claude-mem-file-settings/` - Settings management
...
The troubleshoot skill is designed to be **invoked naturally** - users describe their problem to Claude, and the skill auto-invokes. There is no `/claude-mem-file diagnostics` command.
...
The user's issue is **not** that `/claude-mem-file diagnostics` doesn't work - that command never existed. The actual issues are:
...
```
This will create `~/.claude-mem-file/settings.json` with defaults.

### Step 5: For Diagnostics - Natural Language
Instead of `/claude-mem-file diagnostics`, describe the issue to Claude:
> "I'm having issues with claude-mem-file. Can you help troubleshoot?"
...
Create a `/claude-mem-file` command for diagnostics. File: `plugin/commands/claude-mem-file.json`:
```json
{
...
```javascript
const settingsPath = join(homedir(), '.claude-mem-file', 'settings.json');
if (!existsSync(settingsPath)) {
  mkdirSync(join(homedir(), '.claude-mem-file'), { recursive: true });
...
if (process.platform === 'win32' && !workerStarted) {
  ...
  console.error('And check: https://docs.claude-mem-file.ai/troubleshooting');
}
```
...
1. `/claude-mem-file diagnostics` does not exist - users should use natural language to invoke the troubleshoot skill
...
The fix should include both user documentation improvements and potentially adding a `/claude-mem-file` diagnostic command for discoverability.
