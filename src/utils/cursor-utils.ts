...
export function writeContextFile(workspacePath: string, context: string): void {
  ...
  const rulesFile = join(rulesDir, 'claude-mem-file-context.mdc');
...
---
*Updated after last session. Use claude-mem-file's MCP search tools for more detailed queries.*
`;
...
export function readContextFile(workspacePath: string): string | null {
  const rulesFile = join(workspacePath, '.cursor', 'rules', 'claude-mem-file-context.mdc');
...
/**
 * Configure claude-mem-file MCP server in Cursor's mcp.json
...
/**
 * Remove claude-mem-file MCP server from Cursor's mcp.json
...
export function removeMcpConfig(mcpJsonPath: string): void {
  ...
  try {
    ...
    if (config.mcpServers && config.mcpServers['claude-mem-file']) {
      delete config.mcpServers['claude-mem-file'];
      writeFileSync(mcpJsonPath, JSON.stringify(config, null, 2));
    }
  } catch (e) {
