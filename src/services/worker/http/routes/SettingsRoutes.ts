 ...
 * Settings are stored in ~/.claude-mem-file/settings.json
 */
...
export class SettingsRoutes extends BaseRouteHandler {
  ...
  /**
   * Get environment settings (from ~/.claude-mem-file/settings.json)
   */
  ...
  /**
   * Update environment settings (in ~/.claude-mem-file/settings.json) with validation
   */
  private handleUpdateSettings = this.wrapHandler((req: Request, res: Response): void => {
    ...
    if (existsSync(settingsPath)) {
      ...
      try {
        ...
        res.status(500).json({
          success: false,
          error: 'Settings file is corrupted. Delete ~/.claude-mem-file/settings.json to reset.'
        });
        return;
      }
    }
