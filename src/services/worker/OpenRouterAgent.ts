...
export class OpenRouterAgent {
  ...
  private async queryOpenRouterMultiTurn(
    ...
    const response = await fetch(OPENROUTER_API_URL, {
      ...
      headers: {
        ...
        'HTTP-Referer': siteUrl || 'https://github.com/thedotmack/claude-mem-file',
        'X-Title': appName || 'claude-mem-file',
   ...
   * Issue #733: Uses centralized ~/.claude-mem-file/.env for credentials, not random project .env files
   */
  private getOpenRouterConfig(): { apiKey: string; model: string; siteUrl?: string; appName?: string } {
    const settingsPath = USER_SETTINGS_PATH;
    const settings = SettingsDefaultsManager.loadFromFile(settingsPath);

    // API key: check settings first, then centralized claude-mem-file .env (NOT process.env)
    ...
    const appName = settings.CLAUDE_MEM_OPENROUTER_APP_NAME || 'claude-mem-file';
 ...
 * Issue #733: Uses centralized ~/.claude-mem-file/.env, not random project .env files
 */
