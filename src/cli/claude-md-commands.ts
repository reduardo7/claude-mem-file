 ...
 * - CLI: `claude-mem-file generate` / `claude-mem-file clean`
...
import { VaultStore } from '../services/vault/index.js';
...
function walkDirectoriesWithIgnore(dir: string, folders: Set<string>, depth: number = 0): void {
  ...
  const ignorePatterns = [
    ...
    '.claude-mem-file', '.open-next', '.turbo'
  ];
 ...
 */
function findObservationsByFolder(vault: VaultStore, relativeFolderPath: string, project: string, limit: number): ObservationRow[] {
  const normalizedFolderPath = relativeFolderPath.split(path.sep).join('/');
  const recs = vault.listObservations({
    project,
    orderBy: 'date_desc',
    limit: limit * 3,
  });
  ...
  for (const r of recs) {
    const touchesFolder =
      r.files_modified.some((f) => f.startsWith(`${normalizedFolderPath}/`)) ||
      r.files_read.some((f) => f.startsWith(`${normalizedFolderPath}/`));
    if (!touchesFolder) continue;
    const row: ObservationRow = {
      id: 0, // numeric id not available from vault records; legacy field only used for display
      title: r.title,
      subtitle: r.subtitle,
      narrative: r.narrative,
      facts: r.facts.length ? JSON.stringify(r.facts) : null,
      type: r.type,
      created_at: r.created_at,
      created_at_epoch: r.created_at_epoch,
      files_modified: r.files_modified.length ? JSON.stringify(r.files_modified) : null,
      files_read: r.files_read.length ? JSON.stringify(r.files_read) : null,
      project: r.project,
      discovery_tokens: r.discovery_tokens,
    };
    if (hasDirectChildFile(row, relativeFolderPath)) rows.push(row);
...
function formatObservationsForClaudeMd(observations: ObservationRow[], folderPath: string): string {
  ...
  lines.push('');
  lines.push('<!-- This section is auto-generated by claude-mem-file. Edit content outside the tags. -->');
  lines.push('');
...
function regenerateFolder(
  ...
  try {
    ...
    const observations = findObservationsByFolder(vault, relativeFolder, project, observationLimit);
 ...
 *
 * For each file with <claude-mem-file-context> tags:
  ...
  try {
        ...
        for (const entry of entries) {
          ...
          if (entry.isDirectory()) {
            ...
            try {
              ...
              if (content.includes('<claude-mem-file-context>')) {
    ...
    for (const file of filesToProcess) {
      ...
      try {
        ...
        const stripped = content.replace(/<claude-mem-file-context>[\s\S]*?<\/claude-mem-file-context>/g, '').trim();
